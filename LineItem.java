 ...
 * in a pizza order. The line item contains a link to the application.Pizza object as well as the number
...
public class LineItem implements Comparable<LineItem> {
	 ...
	 * @param pizza The application.Pizza object for this order.
	 * @throws IllegalPizza If the number parameter is not valid.
	 */
	public LineItem(int number, Pizza pizza) throws IllegalPizza {
		setNumber(number);
		if (pizza == null)
			throw new IllegalPizza("application.Pizza not supplied!");
		this.pizza = pizza;
//		this.pizza = pizza.clone();	// If application.Pizza becomes mutable
	} // end full parameter constructor
	
	/**
	 * Overloaded constructor.
	 * @param pizza The application.Pizza object for this order. The order size is assumed to be one.
	...
	/**
	 * The accessor the application.Pizza object.
	 * @return A pointer to the stored application.Pizza object.
	 */
	public Pizza getPizza() {
		return pizza;	// add .clone() if application.Pizza becomes mutable
	} // end getPizza
	...
	/**
	 * Compares the current application.LineItem object to the one supplied. The basis for comparison is the
	 * total cost as returned by getCost().
	 * @param The supplied application.LineItem object.
	 * @return A negative integer if the cost of the supplied application.LineItem object is lower, a positive
...
} // end application.LineItem class
